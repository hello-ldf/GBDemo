	...
	mui.confirm('保存订单，确认？', '提示', btnArray, function(e) {
		...
		if(e.index == 1) {
			...
			mui.ajax({
				...
				data: "{customerJson: " + customerJson + ", corderJson: " + corderJson + ", cordercsJson: " + cordercsJson + "}",
				dataType: "json",
				success: function(data) {
					if(data == "success") {
						mui.alert('保存成功', '提示');
...
function SaveOrder() {
	...
	mui.confirm('保存订单，确认？', '提示', btnArray, function(e) {
		...
		if(e.index == 1) {
			...
			// 获取每个项目的信息
			var dcost = $('#itemCards li .c_dcost');
			var itemNos = $('#itemCards li .c_itemNo');
			var pNos = $('#itemCards li .c_pNo');
			var pColors = $('#itemCards li .c_pColor');
			var stores = $('#itemCards li .c_store');
			...
			$.each(cordercs, function(i, corderc) {
				...
				var corderc = {
					...
					INETFLAG: 1,
					DDEQTY: 0,
					DCOST: dcost[i].innerText,// pd
					DCYPRICE: prices[i].innerText,
					DCYCOST: dcost[i].innerText,
					DAGIOC: 1,
					dshipqty: 0,
					cattrid: attrItems[i].innerText, //pd
					dprice2: price2s[i].innerText,
